...
fn bfs(mut grid: Grid) -> u32 {
    ...
    set.push_back(grid.end_coord);
    let mut end_point: Option<Coord> = None;
    let mut cntr = 0;
    ...
    let mut searcher = end_point.unwrap();
    ...
    loop {
        ...
        match grid.data[h][v].cell_type {
            ...
            CellType::Start => {
                println!("shouldn't encounter start actually");
            }
            CellType::Height(_) => {
